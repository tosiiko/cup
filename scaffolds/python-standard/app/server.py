...
from .actions import ActionResult, update_preferences_action
from .config import HOST, INDEX_FILE, PORT, RUNTIME_FILE, SESSION_ABSOLUTE_SECONDS, SESSION_COOKIE, STATIC_DIR, USE_SECURE_COOKIES
...
class MinimalHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt: str, *args: Any) -> None:
        print(f"[cup-app] {self.command} {self.path} -> {args[0]}")
    ...
    def csrf_error_view(self, session: dict[str, Any]) -> tuple[int, Any]:
        route = ROUTE_FALLBACKS.get(self.path, "/")
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/app.css":
            ...
            return
        if parsed.path == "/cup/index.js":
            self.send_file(RUNTIME_FILE, content_type="text/javascript; charset=utf-8")
            return
    ...
    def do_POST(self) -> None:
        ...
        if self.path == "/api/preferences":
            self.apply_action_result(update_preferences_action(session, payload))
            return
...
def main() -> None:
    if not RUNTIME_FILE.exists():
        raise SystemExit("cup/index.js not found. Re-run `cup init --adapter py-cup --force` to restore the local runtime.")
    ...
    try:
        ...
        if exc.errno == errno.EADDRINUSE:
            raise SystemExit(
                f"Port {PORT} is already in use. Run `CUP_APP_PORT={PORT + 1} python3 server.py` or stop the process using that port."
            ) from exc
        raise

    print(f"CUP Python App -> http://{HOST}:{PORT}")
