...
from .sessions import add_audit_event, create_session, destroy_session, session_expired, SESSIONS
...
class CRMHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt: str, *args: Any) -> None:
        print(f"[crm] {self.command} {self.path} -> {args[0]}")
    ...
    def csrf_error_view(self, session: dict[str, Any], payload: dict[str, Any]) -> tuple[int, Any]:
        error = "Security token validation failed. Refresh and try again."
        if not session.get("authenticated") or self.path == "/api/auth/login":
            return 403, login_view(session, error=error, username=str(payload.get("username", "")).strip())

        route = POST_ROUTE_FALLBACKS.get(self.path, "/crm/overview")
        status, view = resolve_view_for_route(session, route, remote_addr=self.remote_addr(), error=error)
        return max(status, 403), view

    def apply_action_result(self, result: ActionResult) -> None:
        if result.session_id:
            self.pending_cookie = self.make_set_cookie(result.session_id)
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/views":
            ...
            status, view = resolve_view_for_route(session, route, remote_addr=self.remote_addr())
    ...
    def do_POST(self) -> None:
        ...
        if self.path == "/api/auth/login":
            self.apply_action_result(login_action(session_id, session, payload, remote_addr=remote_addr, user_agent=user_agent))
            return
        if self.path == "/api/auth/logout":
            self.apply_action_result(logout_action(session_id, session, remote_addr=remote_addr, user_agent=user_agent))
            return
        if self.path == "/api/leads/create":
            self.apply_action_result(create_lead_action(session, payload, remote_addr=remote_addr))
            return
        if self.path == "/api/security/rotate-session":
            self.apply_action_result(rotate_session_action(session_id, session, remote_addr=remote_addr, user_agent=user_agent))
            return
        if self.path == "/api/security/terminate-others":
            self.apply_action_result(terminate_other_sessions_action(session_id, session, remote_addr=remote_addr))
            return

        self.send_view(login_view(session, error="Unknown POST endpoint."), status=404)
...
def main() -> None:
    ...
    print(f"CUP CRM starter -> http://{HOST}:{PORT}")
